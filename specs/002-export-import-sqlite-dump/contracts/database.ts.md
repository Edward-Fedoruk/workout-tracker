...
4. Guard on `navigator.storage`. If unavailable (OPFS fallback case), skip steps 5–6 and proceed directly to reload — the in-memory DB has no file to swap, so import has no persistent effect; the UI must already warn the user of this before calling.
5. Hand the bytes to `writeDatabaseFile(bytes)` (`src/db/importDatabaseFile.ts`), which spawns the dedicated `importWorker` and resolves/rejects on its reply. Steps 5a–5b happen **inside that worker** — OPFS sync-access writes are worker-only on iOS Safari, where the main thread lacks `createWritable`:
   - 5a. Open the OPFS root and best-effort remove the old file and its sidecars: for each of `app.sqlite3`, `app.sqlite3-journal`, `app.sqlite3-wal`, `app.sqlite3-shm`, call `root.removeEntry(name).catch(() => undefined)` (sidecars commonly don't exist).
   - 5b. Write the new file: `getFileHandle('app.sqlite3', { create: true })` → `createSyncAccessHandle()` (retried briefly while the sqlite worker releases its handle) → `truncate(0)` → `write(bytes, { at: 0 })` → `flush()` → `close()`.
6. If `writeDatabaseFile` rejects (the worker reported an error AFTER step 5a removed files), the caller has been warned in the confirm prompt that an error here leaves the app in a fresh-DB state on next load. The thrown error MUST propagate so the UI can surface it before any reload — `replaceDatabaseAndReload` must NOT call `location.reload()` on failure.
